...
class Trainer:
    ...
    def _train_epoch(self, epoch, loader, penalty_weights):
        ...
        for idx, (src, tgt, lengths, data_index, tgt_str) in enumerate(loader):
            ...
            #print("labels: ", labels)
            loss, CE_loss, KL_loss, pred_logp_loss, pred_sas_loss = self.criterion(output, tgt, mu, sigma, pred_logp, pred_sas, labels_logp, labels_sas, epoch, tgt_str_lst, penalty_weights)
            #pred_loss.backward()
            loss.backward()
