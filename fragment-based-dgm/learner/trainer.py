...
class Trainer:
    ...
    def _train_epoch(self, epoch, loader, penalty_weights, beta):
        ...
        for idx, (src, tgt, lengths, data_index, tgt_str) in enumerate(loader):
            ...
            loss, CE_loss, KL_loss, pred_qed_loss = self.criterion(output, tgt, mu, sigma, pred_qed, labels_qed, labels_logp, labels_sas, epoch, tgt_str_lst, penalty_weights, beta)
            #pred_loss.backward()
            loss.backward()
